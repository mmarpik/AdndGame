...
public class ItemRepository
{
    ...
    public IEnumerable<Item> LoadAll()
    {
        ...
        foreach (var file in Directory.GetFiles(_folder, "*.json"))
        {
            ...
            var grouped = JsonConvert.DeserializeObject<ItemCategoryJsonModel>(json);
            ...
            var single = JsonConvert.DeserializeObject<ItemJsonModel>(json);
            if (single != null)
            {
    ...
    public bool TryAdjustStock(string itemName, int delta)
    {
        ...
        if (grouped?.Items != null && grouped.Items.Count > 0)
        {
            ...
            if (target.StockQuantity.HasValue)
            {
                ...
                var updatedGrouped = JsonConvert.SerializeObject(grouped, Formatting.Indented);
        ...
        if (single.StockQuantity.HasValue)
        {
            ...
            var updated = JsonConvert.SerializeObject(single, Formatting.Indented);
    ...
    private string? FindItemFile(string itemName)
    {
        foreach (var file in Directory.GetFiles(_folder, "*.json"))
        {
            ...
            var model = JsonConvert.DeserializeObject<ItemJsonModel>(json);
    ...
    private static List<CharacterClass> ResolveAllowedClasses(List<string>? rawAllowedClasses)
    {
        ...
        if (hasAll)
            return ((CharacterClass[])Enum.GetValues(typeof(CharacterClass))).ToList();
