...
public class CharacterRepository
{
    ...
    public IEnumerable<Character> GetAll()
    {
        ...
        foreach (var file in Directory.GetFiles(_folder, "*.json"))
        {
            ...
            var c = JsonConvert.DeserializeObject<Character>(json);
            if (c != null)
            {
        ...
        try
        {
            ...
            for (int i = party.Members.Count - 1; i >= 0; i--)
            {
                ...
                if (memberName.TrimStart().StartsWith('{'))
                {
                    try
                    {
                        var root = JObject.Parse(memberName);
                        var nameProp = root["Name"] ?? root["name"];
                        if (nameProp != null && nameProp.Type == JTokenType.String)
                            memberName = (string?)nameProp ?? memberName;
                    }
                    catch
                    {
